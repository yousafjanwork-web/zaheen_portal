...
import { BookOpen, Play, Star, TrendingUp, Target, Zap, ArrowRight, Sparkles, Trophy } from 'lucide-react';
...
export default function Home() {
  ...
  const goToLesson = (lessonId: string) => {
    if (!isLoggedIn) {
      navigate('/login', { state: { from: `/vocab/lesson/${lessonId}` } });
      return;
    }
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <div className="absolute inset-0 opacity-30">
          ...
          <div className="absolute bottom-0 left-0 w-60 h-60 bg-cyan-300 rounded-full blur-3xl animate-float" style={{ animationDelay: '1s' }} />
        </div>
        {/* Grid overlay */}
        <div
          aria-hidden
        ...
        {['✨', '⭐', '🌟', '💫'].map((s, i) => (
          <motion.div
            ...
            animate={{ y: [0, -15, 0], rotate: [0, 20, -20, 0], opacity: [0.4, 0.9, 0.4] }}
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-4 flex-1">
            ...
            <p className="text-white/85 text-lg max-w-md leading-relaxed">
              Ready to learn new words and become a vocabulary champion? Your next adventure awaits! 🚀
            </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Current Level', value: currentLevel, icon: Trophy, color: 'from-amber-400 to-orange-500' },
          { label: 'Total XP', value: `${user.xp.toLocaleString()}`, icon: Star, color: 'from-blue-500 to-indigo-600' },
          { label: 'Lessons Done', value: `${user.lessonsCompleted}`, icon: Target, color: 'from-lime-400 to-green-500' },
          { label: 'Words Learned', value: `${user.wordsLearned}`, icon: Zap, color: 'from-rose-400 to-pink-500' },
          ...
          >
            ...
            {/* Decorative blur */}
            <div className={`absolute -top-6 -right-6 w-20 h-20 rounded-full bg-gradient-to-br ${color} opacity-10 blur-2xl group-hover:opacity-20 transition-opacity`} />

            <div className={`relative w-11 h-11 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center mb-3 shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-transform`}>
              <Icon className="w-5 h-5 text-white" />
            </div>
            <p className="text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold">{label}</p>
            <p className="text-xl font-black text-slate-800 dark:text-white mt-1 truncate">{value}</p>
          </motion.div>
        ))}
      </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              {nextLevel ? `${user.xp.toLocaleString()} / ${xpForNextLevel.toLocaleString()} XP to "${nextLevel}"` : 'Maximum level reached! 🎉'}
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-lime-700 dark:text-lime-400">Today's Lesson</span>
              <h3 className="font-black text-lg text-slate-800 dark:text-white mt-0.5">
                {nextLesson?.title || 'No lessons available'}
              </h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                Learn {nextLesson?.words.length || 0} new vocabulary words today!
              </p>
             <button
                onClick={() => goToLesson(nextLesson?.id || 'lesson-1')}
        ...
        >
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-400">Weekly Stats</span>
              <h3 className="font-black text-lg text-slate-800 dark:text-white mt-0.5">
                {user.streak}-Day Streak 🔥
              </h3>
              <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { to: '/vocab/flashcards', label: 'Flashcards', sub: 'Quick review', icon: '🃏', color: 'from-cyan-500 to-blue-500' },
            { to: '/vocab/story-studio', label: 'Story Studio', sub: 'Write & publish', icon: '✍️', color: 'from-pink-500 to-rose-500' },
            { to: '/vocab/word-garden', label: 'Word Garden', sub: 'Grow words', icon: '🌱', color: 'from-lime-500 to-green-500' },
            { to: '/vocab/leaderboard', label: 'Leaderboard', sub: 'Compete!', icon: '🏆', color: 'from-amber-500 to-orange-500' },
            { to: '/vocab/quests', label: 'Daily Quests', sub: 'Bonus XP', icon: '🎯', color: 'from-purple-500 to-fuchsia-500' },
          ].map((a, i) => (
            <motion.div
              key={a.to}
              initial={{ opacity: 0, y: 20 }}
            ...
            >
              ...
              >
                <div className={`absolute -top-6 -right-6 w-16 h-16 rounded-full bg-gradient-to-br ${a.color} opacity-20 blur-2xl group-hover:opacity-30 transition-opacity`} />
                <div className="relative">
                  <div className="text-3xl mb-2">{a.icon}</div>
                  <p className="font-black text-sm text-slate-800 dark:text-white">{a.label}</p>
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider font-bold">{a.sub}</p>
                </div>
              </Link>
            </motion.div>
          ))}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/vocab/courses" className="text-sm font-bold text-blue-600 dark:text-blue-400 hover:text-lime-600 transition-colors inline-flex items-center gap-1">
            ...
            >
              ...
              <div className="relative">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 line-clamp-2">
                  {lesson.words.length} new words • {lesson.words.slice(0, 3).map(w => w.word).join(', ')}{lesson.words.length > 3 ? '…' : ''}
                </p>
