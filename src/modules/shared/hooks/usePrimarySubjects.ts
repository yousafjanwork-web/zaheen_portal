/**
 * usePrimarySubjects.ts — PRIMARY (Grade 1–5)-ONLY hook
 *
 * Drop-in replacement for useClassSubjects, used ONLY inside
 * PrimarySubjectsView.tsx and PrimarySubjectDetailView.tsx. The shared
 * useClassSubjects.ts (still used by Grade 6–8, 9–12) is NOT touched.
 *
 * Return shape matches what PrimarySubjectsView/PrimarySubjectDetailView
 * already destructure: { classInfo, subjects, chapters, chapterVideos,
 * loading, error } — same contract as useKGSubjects.ts.
 */
import { useEffect, useState, useRef } from "react";
import {
  fetchPrimaryClasses,
  fetchPrimarySubjects,
  fetchPrimaryChapters,
  fetchPrimaryVideos,
  fetchPrimaryVideoDetail,
} from "@/modules/shared/services/primaryService";

/**
 * Re-exported so PrimarySubjectDetailView can fetch the real playable
 * video_url at selection time (the list endpoint used below does not
 * include it — confirmed by direct testing, same as KG).
 */
export { fetchPrimaryVideoDetail };
...
interface UsePrimarySubjectsResult {
...
/* ─────────────────────────────────────────────────────────────
   Hook
──────────────────────────────────────────────────────────────── */
export const usePrimarySubjects = (classId: number): UsePrimarySubjectsResult => {
      ...
      try {
        ...
        await Promise.all(
          normalizedSubjects.map(async (subject: NormalizedSubject) => {
            try {
              const chaptersRaw = await fetchPrimaryChapters(subject.id);
              ...
              await Promise.all(
                normalizedChaps.map(async (chapter: NormalizedChapter) => {
                  try {
                    const videosRaw = await fetchPrimaryVideos(chapter.id);
