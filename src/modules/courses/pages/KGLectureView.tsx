...
import {
  BookOpen, CheckCircle2, ChevronLeft, ChevronRight, ArrowLeft, Play, Clock, LayoutGrid,
} from "lucide-react";
...
interface Video {
  ...
  thumbnail?: string; thumb?: string; image?: string; cover?: string; poster?: string;
  [key: string]: any;
}
...
const getTheme = (name: string): SubjectTheme => {
  ...
  if (n.includes("english")) return {
    variant: "english", heroBg: "#C7D2FE", heroBgHex: "#C7D2FE",
    heroTextColor: "text-slate-900", heroBreadcrumb: "text-slate-600",
    heroImage: owl, mascotFallback: "🦉",
    progressAccent: "#4F46E5", cardBorderColor: "#10B981",
    taglineKey: "kgLectureView.taglines.english",
    accentHex: "#4F46E5",
    badgeKey: "kgLectureView.hero.kgBadge", badgeColor: "#10B981",
    lectureColors: ["bg-[#6EE7B7] text-green-900","bg-[#BFDBFE] text-blue-900","bg-[#FED7AA] text-orange-900","bg-[#DDD6FE] text-purple-900"],
  };

  if (n.includes("urdu")) return {
    variant: "urdu", heroBg: "#F97316", heroBgHex: "#F97316",
    heroTextColor: "text-white", heroBreadcrumb: "text-orange-100",
    heroImage: orange, mascotFallback: "✍️",
    progressAccent: "#16A34A", cardBorderColor: "#F97316",
    taglineKey: "kgLectureView.taglines.urdu",
    accentHex: "#EA580C",
    lectureColors: ["bg-[#FED7AA] text-orange-900","bg-[#FEF3C7] text-yellow-900","bg-[#FCE7F3] text-pink-900","bg-[#FFEDD5] text-orange-900"],
  };

  if (n.includes("math")) return {
    variant: "math", heroBg: "#BBF7D0", heroBgHex: "#BBF7D0",
    heroTextColor: "text-slate-900", heroBreadcrumb: "text-green-700",
    heroImage: one, mascotFallback: "🔢",
    progressAccent: "#16A34A", cardBorderColor: "#10B981",
    taglineKey: "kgLectureView.taglines.math",
    accentHex: "#059669",
    lectureColors: ["bg-[#BBF7D0] text-green-900","bg-[#A7F3D0] text-emerald-900","bg-[#D1FAE5] text-green-900","bg-[#ECFDF5] text-green-900"],
  };

  if (n.includes("islamic")) return {
    variant: "default", heroBg: "#0D9488", heroBgHex: "#0D9488",
    heroTextColor: "text-white", heroBreadcrumb: "text-teal-100",
    mascotFallback: "🌙",
    progressAccent: "#0D9488", cardBorderColor: "#0D9488",
    taglineKey: "kgLectureView.taglines.islamic",
    accentHex: "#0D9488",
    lectureColors: ["bg-[#CCFBF1] text-teal-900","bg-[#99F6E4] text-teal-900","bg-[#A7F3D0] text-green-900","bg-[#BAE6FD] text-sky-900"],
  };

  if (n.includes("pakistan")) return {
    variant: "default", heroBg: "#FDE68A", heroBgHex: "#FDE68A",
    heroTextColor: "text-slate-900", heroBreadcrumb: "text-yellow-700",
    mascotFallback: "🌍",
    progressAccent: "#D97706", cardBorderColor: "#D97706",
    taglineKey: "kgLectureView.taglines.pakistan",
    accentHex: "#D97706",
    lectureColors: ["bg-[#FDE68A] text-yellow-900","bg-[#FED7AA] text-orange-900","bg-[#FEF3C7] text-yellow-900","bg-[#FFEDD5] text-orange-900"],
  };

  if (n.includes("computer") || n.includes("cs")) return {
    variant: "default", heroBg: "#4338CA", heroBgHex: "#4338CA",
    heroTextColor: "text-white", heroBreadcrumb: "text-indigo-200",
    mascotFallback: "🤖",
    progressAccent: "#4338CA", cardBorderColor: "#4338CA",
    taglineKey: "kgLectureView.taglines.computer",
    accentHex: "#4338CA",
    lectureColors: ["bg-[#E0E7FF] text-indigo-900","bg-[#C7D2FE] text-indigo-900","bg-[#DDD6FE] text-purple-900","bg-[#EDE9FE] text-violet-900"],
  };

  return {
    variant: "default", heroBg: "#4F46E5", heroBgHex: "#4F46E5",
    heroTextColor: "text-white", heroBreadcrumb: "text-indigo-200",
    mascotFallback: "📚",
    progressAccent: "#4F46E5", cardBorderColor: "#4F46E5",
...
const ProgressBar = ({
  pct, progressAccent,
}: { pct: number; progressAccent: string; variant?: "full" | "compact" | "embedded" }) => (
...
═══════════════════════════════════════════════════════════ */
const HeroEnglish = ({
  subjectName, gradeName, theme, totalVideos, watchedCount, isRtl,
  onBack, onBreadcrumbGradeClick, t,
}: {
  subjectName: string; gradeName: string; theme: SubjectTheme;
  totalVideos: number; watchedCount: number; isRtl: boolean;
  onBack: () => void; onBreadcrumbGradeClick: () => void;
    ...
    <div className="mb-8 px-4 sm:px-6 lg:px-8 pt-6 max-w-[1070px] mx-auto">
      ...
      >
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center">
          ...
          <p className={`text-[15px] sm:text-[17px] text-slate-700 leading-relaxed max-w-xl mb-8 ${isRtl ? "text-right" : ""}`}>{tagline}</p>
        ...
        <div className="relative z-10 shrink-0 flex items-center justify-center">
          {theme.heroImage ? (
            ...
            <div className="text-[130px] leading-none select-none" style={{ animation: "kgHeroFloat 3.5s ease-in-out infinite" }}>{theme.mascotFallback}</div>
          )}
        </div>
      </div>
        ...
        <div className="relative z-10 max-w-[1100px] mx-auto px-6 sm:px-10 lg:px-14 pt-10 pb-28">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8">
            <div className="flex-1 min-w-0">
              <h1 className="text-[38px] sm:text-[52px] font-black leading-none tracking-tight mb-4 text-white" style={{ fontFamily: FONT }}>
                {subjectName}
              </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-10 sm:gap-6">
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-[11px] font-black uppercase tracking-[0.18em]" style={{ color: theme.accentHex, fontFamily: FONT }}>
                  {t("kgLectureView.hero.mathematics")}
                </p>
              </div>
              <h1 className="text-[36px] sm:text-[48px] font-black leading-tight text-slate-900 mb-4" style={{ fontFamily: FONT }}>
                {subjectName}
              </h1>
...
═══════════════════════════════════════════════════════════ */
const HeroDefault = ({
  subjectName, gradeName, theme, totalVideos, watchedCount, isRtl, onBack, t,
}: {
  subjectName: string; gradeName: string; theme: SubjectTheme;
  totalVideos: number; watchedCount: number; isRtl: boolean;
  onBack: () => void;
      ...
      <div className="relative overflow-hidden" style={{ backgroundColor: theme.heroBgHex }}>
        ...
        <div className="relative z-10 max-w-[1100px] mx-auto px-6 sm:px-10 lg:px-14 pt-10 pb-28 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[13px] font-semibold mb-5 ${theme.heroBreadcrumb}`}>
              {t("kgLectureView.home")} &nbsp;›&nbsp; {gradeName || t("kgClassView.defaultGrade")}
            </p>
            <h1 className={`text-[38px] sm:text-[52px] font-black leading-none tracking-tight mb-5 ${theme.heroTextColor}`} style={{ fontFamily: FONT }}>
              {subjectName}
            </h1>
...
──────────────────────────────────────────────────────────────── */
const VideoCard = ({
  video, index, isWatched, progress, isPlaying, theme, isRtl, onClick, t,
}: {
  video: Video; index: number; isWatched: boolean; progress: number;
  isPlaying: boolean; theme: SubjectTheme; isRtl: boolean; onClick: () => void;
  t: ReturnType<typeof useT>;
}) => {
  ...
  const pillColor = theme.lectureColors[(index - 1) % theme.lectureColors.length];

  const rawThumb = video[VIDEO_THUMB_FIELD] || video.thumb || video.image || video.cover || video.poster || null;
  ...
  return (
    <motion.div
      ...
      style={{ borderTop: `4px solid ${theme.cardBorderColor}`, outline: isPlaying ? `2px solid ${theme.accentHex}` : undefined, outlineOffset: isPlaying ? "2px" : undefined }}
    >
      <div className="relative w-full overflow-hidden bg-slate-200" style={{ aspectRatio: "16/9" }}>
        <img src={thumbSrc} alt={title} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = fallbackThumbnail; }} />
      ...
      <div className="p-5">
        ...
        {shortDesc && (
          <p className={`text-[14px] text-slate-500 leading-relaxed line-clamp-3 mb-4 ${isRtl ? "text-right" : ""}`}>{shortDesc}</p>
        )}
        <div className="flex items-center justify-between mt-auto">
          ...
          <button onClick={(e) => { e.stopPropagation(); onClick(); }} className="w-10 h-10 rounded-full flex items-center justify-center text-white shadow-md transition-transform hover:scale-110" style={{ backgroundColor: theme.accentHex }}>
...
──────────────────────────────────────────────────────────────── */
const VideoPlayer = ({
  video, videoUrl, lectureIndex, totalLectures, theme, isRtl,
  gradeName, onBack, onBreadcrumbGradeClick,
  onEnded, onTimeUpdate, onLoaded, onPlay, onClose, onNext, onPrev, videoRef, t,
}: {
  video: Video; videoUrl: string; lectureIndex: number; totalLectures: number;
  theme: SubjectTheme; isRtl: boolean;
  gradeName: string; onBack: () => void; onBreadcrumbGradeClick: () => void;
  onEnded: () => void; onTimeUpdate: (e: React.SyntheticEvent<HTMLVideoElement>) => void;
  onLoaded: () => void; onPlay: () => void; onClose: () => void;
  onNext: () => void; onPrev: () => void; videoRef: React.RefObject<HTMLVideoElement>;
  t: ReturnType<typeof useT>;
}) => {
  const title   = isRtl ? video.urdu_name || video.name : video.name;
  const descRaw = isRtl ? video.urdu_desc || video.desc : video.desc;
    ...
    >
      ...
      <div className="bg-black rounded-3xl overflow-hidden shadow-2xl">
        <div className="aspect-video">
          <video
            ref={videoRef} key={videoUrl} src={videoUrl}
            controls autoPlay className="w-full h-full"
            onLoadedData={onLoaded} onPlay={onPlay}
...
const ChapterSection = ({
  chapter, chapterIndex, globalOffset, selectedVideo, watchedSet,
  progressMap, theme, isRtl, onSelect, sectionRef, t,
}: {
  chapter: ChapterWithVideos; chapterIndex: number; globalOffset: number;
  selectedVideo: Video | null; watchedSet: Set<number>; progressMap: Record<number, number>;
  theme: SubjectTheme; isRtl: boolean; onSelect: (v: Video) => void;
    ...
    <div ref={sectionRef} id={`chapter-${chapter.id}`} className="mb-12 scroll-mt-4">
      {chapter.name && (
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <h3 className={`text-[17px] font-black text-slate-900 leading-tight ${isRtl ? "text-right" : ""}`} style={{ fontFamily: FONT }}>{chLabel}</h3>
          </div>
          {watched > 0 && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {chapter.videos.map((video, localIdx) => {
          ...
          return (
            <VideoCard
              key={video.id} video={video} index={globalIdx + 1}
              ...
              theme={theme} isRtl={isRtl} onClick={() => onSelect(video)} t={t}
            />
          );
        })}
      </div>
...
const KGLectureView = () => {
  ...
  const subjectName    = isRtl ? selectedSubject?.urdu_name || subjectRawName : subjectRawName;
  ...
  const trackEvent = useCallback((eventName: string) => {
    ...
    window.gtag("event", eventName, { video_id: selectedVideo.id, video_name: selectedVideo.name });
  }, [selectedVideo]);
  ...
  const goNext = useCallback(() => {
    if (currentGlobalIdx < allVideos.length - 1) selectVideo(allVideos[currentGlobalIdx + 1]);
  }, [currentGlobalIdx, allVideos, selectVideo]);

  const goPrev = useCallback(() => {
    if (currentGlobalIdx > 0) selectVideo(allVideos[currentGlobalIdx - 1]);
  }, [currentGlobalIdx, allVideos, selectVideo]);
  ...
  const closePlayer = useCallback(() => {
    setSelectedVideo(null); setVideoUrl("");
    window.scrollTo({ top: 0, behavior: "smooth" });
  }, []);
  ...
  const renderHero = () => {
    ...
    switch (theme.variant) {
      case "english": return <HeroEnglish {...commonProps} />;
      case "urdu":    return <HeroUrdu    {...commonProps} />;
      case "math":    return <HeroMath    {...commonProps} />;
      default:        return <HeroDefault {...commonProps} onBack={handleBackToGrade} />;
    }
  };
    ...
    <section className="bg-[#F1F5F9] min-h-screen overflow-x-hidden">
      ...
      <div className={`max-w-[1100px] mx-auto px-6 sm:px-10 lg:px-14 pb-12 ${selectedVideo ? "pt-8" : ""}`}>
        ...
        <AnimatePresence mode="wait">
          {selectedVideo && (
            <VideoPlayer
              ...
              video={selectedVideo} videoUrl={videoUrl}
              lectureIndex={currentGlobalIdx + 1} totalLectures={allVideos.length}
              theme={theme} isRtl={isRtl}
              gradeName={gradeName}
              onBack={handleBackToHome}
              onBreadcrumbGradeClick={handleBackToGrade}
              onEnded={handleEnded} onTimeUpdate={handleTimeUpdate}
              onLoaded={handleLoaded} onPlay={handlePlay}
              onClose={closePlayer} onNext={goNext} onPrev={goPrev}
        ...
        {!loading && subjectChapters.map((chapter, chIdx) => (
          <ChapterSection
            key={chapter.id}
            chapter={chapter} chapterIndex={chIdx} globalOffset={chapterOffsets[chIdx]}
            selectedVideo={selectedVideo} watchedSet={watchedSet} progressMap={progressMap}
            theme={theme} isRtl={isRtl} onSelect={selectVideo} t={t}
