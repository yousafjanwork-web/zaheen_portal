    ...
    for (const part of parts) {
      if (node && typeof node === "object" && part in (node as Record<string, unknown>)) node = (node as Record<string, unknown>)[part];
        ...
        for (const p of parts) {
          if (fallback && typeof fallback === "object" && p in (fallback as Record<string, unknown>)) fallback = (fallback as Record<string, unknown>)[p];
          else return key;
        }
...
interface SubjectMeta {
  ...
  accentColor: string; lessonPillBg: string; tag: string;
}
...
const getPrimaryMeta = (name: string, isUrdu: boolean = false): SubjectMeta => {
  ...
  if (n.includes("english"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#EEF2FF 0%,#E0E7FF 100%)", iconBg: "#E0E7FF", iconColor: "#3730A3",
  ...
  if (n.includes("urdu"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#FFF7ED 0%,#FFEDD5 100%)", iconBg: "#FFEDD5", iconColor: "#92400E",
  ...
  if (n.includes("math"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#ECFDF5 0%,#D1FAE5 100%)", iconBg: "#D1FAE5", iconColor: "#065F46",
  ...
  if (n.includes("science") || n.includes("bio") || n.includes("chem"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#EFF6FF 0%,#DBEAFE 100%)", iconBg: "#DBEAFE", iconColor: "#1D4ED8",
  ...
  if (n.includes("social") || n.includes("pak") || n.includes("civics"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#EEF2FF 0%,#E0E7FF 100%)", iconBg: "#E0E7FF", iconColor: "#4338CA",
  ...
  if (n.includes("islamic") || n.includes("quran") || n.includes("din"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#F0FDF4 0%,#DCFCE7 100%)", iconBg: "#DCFCE7", iconColor: "#166534",
  ...
  if (n.includes("computer") || n.includes("ict") || n.includes("tech"))
    return {
      ...
      cardBg: "linear-gradient(145deg,#F5F3FF 0%,#EDE9FE 100%)", iconBg: "#EDE9FE", iconColor: "#6D28D9",
  ...
  return {
    ...
    cardBg: "linear-gradient(145deg,#F8FAFC 0%,#F1F5F9 100%)", iconBg: "#E2E8F0", iconColor: "#475569",
    accentColor: "#475569", lessonPillBg: "#CBD5E1", tag: "CURRICULUM",
  };
};

const getLectureCount = (subjectId: number, chapters: any[], chapterVideos: Record<number, any[]>, loading: boolean): { count: number; isLoading: boolean } => {
  const subjectChapters = chapters.filter((c: any) => c.subject_id === subjectId);
  const inFlight = subjectChapters.length > 0 && subjectChapters.every((c: any) => !(c.id in chapterVideos));
  if (loading || inFlight) return { count: 0, isLoading: true };
  let count = 0;
  subjectChapters.forEach((c: any) => { count += (chapterVideos[c.id] || []).length; });
...
const PrimarySubjectCard = ({
  ...
  // ✅ Pass isUrdu so description switches language
  ...
  const lessonLabel = lectureLoading ? "Loading..." : lectureCount > 0 ? `${lectureCount} Lessons` : "Coming Soon";
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 p-6 flex-1">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm" style={{ background: "#ffffff" }}>
          <Icon size={26} style={{ color: meta.iconColor }} strokeWidth={1.8} />
        </div>
        <div className="flex-1">
          <h3 className={`text-[20px] font-black text-[#0F172A] leading-snug mb-2 ${isUrdu ? "text-right" : ""}`}>{title}</h3>
          <p className={`text-[13.5px] text-slate-500 leading-relaxed line-clamp-3 ${isUrdu ? "text-right" : ""}`}>{meta.description}</p>
        </div>
        <div className="flex items-center justify-between mt-2">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">{meta.tag}</span>
            <span className="text-[13px] font-bold px-3 py-1 rounded-full inline-block" style={{ background: meta.lessonPillBg, color: meta.iconColor }}>
              {lessonLabel}
            </span>
          </div>
          <button
...
/* ── Fun Games Card ── */
const FunGamesCard = ({ navigate, isUrdu, index }: { navigate: ReturnType<typeof useNavigate>; isUrdu: boolean; index: number }) => (
  <motion.div
    initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.35, delay: index * 0.06 }}
    onClick={() => navigate("/games")}
    className="group cursor-pointer rounded-3xl overflow-hidden flex flex-col"
    style={{ background: "linear-gradient(145deg,#EEF2FF 0%,#E0E7FF 100%)", border: "1.5px solid rgba(0,0,0,0.06)" }}
  >
    <div className="flex flex-col gap-4 p-6 flex-1">
      <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm" style={{ background: "#ffffff" }}>
      ...
      <div className="flex-1">
        <h3 className={`text-[20px] font-black text-[#0F172A] leading-snug mb-2 ${isUrdu ? "text-right" : ""}`}>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">INTERACTIVE</span>
          <span className="text-[13px] font-bold px-3 py-1 rounded-full inline-block" style={{ background: "#C7D2FE", color: "#3730A3" }}>
...
/* ── Daily Quizzes Card ── */
const DailyQuizzesCard = ({ navigate, isUrdu, classId, gradeType, index }: {
  ...
  <motion.div
    ...
    className="group cursor-pointer rounded-3xl overflow-hidden flex flex-col"
    style={{ background: "linear-gradient(145deg,#FFF1F2 0%,#FFE4E6 100%)", border: "1.5px solid rgba(0,0,0,0.06)" }}
  >
    <div className="flex flex-col gap-4 p-6 flex-1">
      <div className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm" style={{ background: "#ffffff" }}>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-black tracking-widest text-slate-400 uppercase">ASSESSMENT</span>
          <span className="text-[13px] font-bold px-3 py-1 rounded-full inline-block" style={{ background: "#FECDD3", color: "#BE123C" }}>
            {isUrdu ? "روزانہ نیا" : "New Daily"}
          </span>
        </div>
        <button
          onClick={(e) => { e.stopPropagation(); navigate(`/class/${classId}/quiz`, { state: { gradeType } }); }}
          className="w-11 h-11 rounded-full flex items-center justify-center shadow-md transition-transform group-hover:scale-110"
...
const AiTutorCard = ({ navigate, isUrdu }: { navigate: ReturnType<typeof useNavigate>; isUrdu: boolean }) => (
  <motion.div
    initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}
  ...
  >
    ...
    <div className="flex-1">
      <h3 className={`text-[20px] font-black text-white leading-snug mb-2 ${isUrdu ? "text-right" : ""}`}>
        {isUrdu ? "اے آئی ٹیوٹر" : "AI Tutor"}
      </h3>
      <p className={`text-[13.5px] text-slate-400 leading-relaxed ${isUrdu ? "text-right" : ""}`}>
        {isUrdu
...
/* ── Skeleton ── */
const CardSkeleton = () => (
  <div className="rounded-3xl bg-slate-100 animate-pulse overflow-hidden flex flex-col p-6 gap-4" style={{ minHeight: 260 }}>
...
const PrimarySubjectsView = () => {
  ...
  const { classInfo, subjects, chapterVideos, chapters, loading } = useClassSubjects(Number(classId));
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F8FAFC", fontFamily: "'Nunito', 'Segoe UI', sans-serif", direction: isUrdu ? "rtl" : "ltr" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-10 py-10">
        ...
        <div className="text-sm text-slate-400 flex items-center gap-2 mb-7">
          ...
          <span>›</span>
          <Link to="/grade-view/1-5" className="hover:text-slate-600 transition-colors">{isUrdu ? "گریڈ ۱-۵" : "Grades 1–5"}</Link>
          <span>›</span>
        ...
        {/* Header */}
        <div className="mb-9">
          <h1 className="text-[36px] font-black text-[#0F172A] tracking-tight leading-none mb-3">{gradeName}</h1>
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              <p className="text-lg font-semibold">{isUrdu ? "کوئی مضمون نہیں ملا" : "No subjects found"}</p>
            </motion.div>
            ...
            <motion.div key="cards" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
              ...
              {sortedSubjects.map((subject: any, i: number) => {
                const { count, isLoading } = getLectureCount(subject.id, chapters, chapterVideos, loading);
                return (
                  <PrimarySubjectCard
                    key={subject.id} subject={subject} classInfo={classInfo}
                    gradeType={gradeType || "1-5"} navigate={navigate}
                    isUrdu={isUrdu} index={i} lectureCount={count} lectureLoading={isLoading}
                  />
                );
              })}

              <FunGamesCard navigate={navigate} isUrdu={isUrdu} index={sortedSubjects.length} />
              <DailyQuizzesCard navigate={navigate} isUrdu={isUrdu} classId={classId} gradeType={gradeType} index={sortedSubjects.length + 1} />
