...
// ── All heroTitle values now have both `en` and `ur` keys ──
...
const PrimarySubjectDetailView = () => {
  ...
  // ── Pick the correct language for heroTitle and tagline ──
  ...
  const handleVideoEnded = () => {
    if (activeVideo) {
      setWatchedSet((p) => new Set(p).add(activeVideo.id));
      setProgressMap((p) => ({ ...p, [activeVideo.id]: 100 }));
    }
  ...
  if (activeVideo && activeChapter) {
    ...
    const videoUrl   = `${CDN}${activeVideo.path}`;
        ...
        <style>{`
          ...
          *, *::before, *::after { box-sizing: border-box; }
          ...
          .psv-back {
            ...
            color: ${theme.color}; font-weight: 700; font-size: .9rem;
            background: none; border: none; cursor: pointer;
            font-family: 'Nunito',sans-serif; padding: 0; transition: opacity .15s;
          }
          ...
          .psv-next-item.locked { cursor: default; pointer-events: none; }
          .psv-next-item.locked .psv-next-thumb { opacity: .5; }

          @media (max-width: 1024px) {
            .psv-player-grid { grid-template-columns: 1fr; }
          }
          @media (max-width: 860px) {
            .psv-info-row { grid-template-columns: 1fr 1fr; }
            .psv-box-goal { grid-column: 1 / -1; }
          }
          @media (max-width: 560px) {
            .psv-info-row { grid-template-columns: 1fr; }
            .psv-box-goal { grid-column: auto; }
            .psv-player-outer { padding: 16px 14px 40px; }
          }
        `}</style>
          ...
          <div className="psv-player-grid">
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div className="psv-info-row">
                ...
                {/* BOX 1 — Description + Buttons */}
                <div className="psv-info-box psv-box-desc">
                  <div style={{ flex:1 }}>
                    {/* ── heroTitle is now bilingual ── */}
                    ...
                    <p style={{ fontSize:".82rem", color:"#6B7280", lineHeight:1.65, margin:0 }}>
                      {isUrdu
                        ? "چھپے ہوئے حروف تلاش کریں۔ پڑھنے کے پہلے قدموں کے لیے بہترین!"
                        : "Join us on an adventure to find the hidden letters in the Enchanted Forest! Perfect for first steps into reading."}
                    </p>
                  </div>
                ...
                <div className="psv-info-box psv-box-robot">
                  ...
                  <div style={{
                    background:"#F8FAFC", borderRadius:13,
                ...
                <div className="psv-info-box psv-box-goal" style={{ background:theme.light, borderColor:theme.pill }}>
                  ...
                  <div>
                    <div style={{ background:"rgba(0,0,0,.1)", borderRadius:100, height:12, overflow:"hidden", marginBottom:10 }}>
                      <div style={{ height:"100%", width:`${Math.min(progressPct + 20, 100)}%`, background:theme.color, borderRadius:100, transition:"width .4s ease" }} />
                    </div>
                    <p style={{ fontSize:".82rem", color:theme.color, fontWeight:800, margin:"0 0 5px" }}>
                      {isUrdu ? "بہت اچھا! ایک اور ویڈیو باقی ہے!" : "Almost there! One more video to go!"}
                    </p>
              ...
              <div style={{ background:"#fff", borderRadius:18, padding:"18px 16px", boxShadow:"0 2px 12px rgba(0,0,0,.06)", position:"sticky", top:24 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
                  ...
                  {upcomingVideos.map(({ video, chapter, globalIdx }) => {
                    ...
                    return (
                      <div key={video.id} className={`psv-next-item${locked ? " locked" : ""}`}
                        onClick={() => !locked && playVideo(video, chapter)}>
                        <div className="psv-next-thumb" style={{
                          width:82, height:54, borderRadius:10, overflow:"hidden",
                          background:"#E9EEF6", flexShrink:0,
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        @media (max-width:1200px) {
          .psv-hero { grid-template-columns:1fr 1fr; min-height:320px; }
        }
        @media (max-width:1024px) {
          .psv-layout { grid-template-columns: 280px 1fr; }
          .psv-hero { grid-template-columns:1fr 340px; min-height:280px; }
          .psv-hero-left { padding:36px 30px; }
        }
        @media (max-width:860px) {
          .psv-layout { grid-template-columns: 1fr; }
          .psv-sidebar { position:static; max-height:none; }
          .psv-hero { grid-template-columns:1fr; min-height:auto; }
          .psv-hero-right { height:220px; }
          .psv-hero-left { padding:28px 24px; gap:12px; }
        }
        @media (max-width:600px) {
          .psv-lec-grid { grid-template-columns:1fr; }
          .psv-hero-right { height:180px; }
        }
      `}</style>
        ...
        {loading ? (
          ...
          <div className="psv-hero" style={{ background: theme.light }}>
            <div className="psv-hero-left">
              ...
              {/* ── heroTitle now switches language ── */}
              ...
              <>
                ...
                {activeChapterData.videos.length === 0 ? (
                  ...
                  <div className="psv-lec-grid">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="psv-lec-body">
                            <p className="psv-lec-chapter-tag" style={{ color: theme.color }}>{chapterLabel}</p>
                    ...
                    {/* Quiz Game card */}
                    <div style={{ borderRadius:18, border:"2px dashed #D1FAE5", background:"#F0FDF4", padding:"24px 20px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:12, cursor:"pointer", textAlign:"center" }}
                      onClick={() => navigate("/class/${classId}/quiz")}>
