...
import PrimaryGradesView from "./PrimaryGradesView";
import MiddleGradesView  from "./MiddleGradesView";
...
const GradesView = () => {
  ...
  // ✅ 1. SAARE HOOKS TOP PAR: useGrades ko upar le aaye taake har render par lazmi execute ho
    ...
    <section className="py-20 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6 text-sm text-slate-500 flex gap-2">
          ...
          <span>/</span>
          <span className="text-slate-700 font-medium">{isUrdu ? title.ur : title.en}</span>
        </div>
        <WelcomeCard isUrdu={isUrdu} title={title} />
        <div className="flex flex-col lg:flex-row gap-10">
          <Sidebar isLoggedIn={isLoggedIn} isUrdu={isUrdu} type={type} navigate={navigate} />
          <div className="flex-1">
            {loading ? (
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                {Array.from({ length: 6 }).map((_, i) => <GradeCardSkeleton key={i} />)}
              </div>
            ) : (
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                {grades.map((g, i) => <GradeCard key={i} grade={g} navigate={navigate} type={type} />)}
              </div>
            )}
          </div>
        </div>
