...
import { fetchChapters, fetchVideos } from "@/modules/shared/services/classService";
import { useAuth } from "@/modules/shared/context/AuthContext";
import { useVideoProgress } from "@/modules/shared/hooks/Usevideoprogress";
import {
...
/* ─────────────────────────────────────────────────────────────
   useGradeVideoIds
   Fetches all video IDs for a given classId so we can pass them
   to useVideoProgress and get REAL server-side progress.
──────────────────────────────────────────────────────────────── */
function useGradeVideoIds(classId: number, subjects: any[]): number[] {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // For each subject → get chapters → get videos
      await Promise.all(
  subjects.map(async (subject: any) => {
    try {
      const chapters: any[] = await fetchChapters(subject.id);

      await Promise.all(
        chapters.map(async (ch: any) => {
          try {
            const videos: any[] = await fetchVideos(ch.id);
            videos.forEach((v: any) => { if (v?.id) allIds.push(v.id); });
          } catch { /* chapter failed — skip */ }
        })
      );
    } catch { /* subject failed — skip */ }
  })
);

        if (!cancelled) setVideoIds(allIds);
      } catch { /* outer failure — leave empty */ }
    })();

    return () => { cancelled = true; };
  }, [classId, subjects.map((s: any) => s.id).join(",")]);

  return videoIds;
}

/* ─────────────────────────────────────────────────────────────
   GradeCard — shows REAL progress from the v2 API
──────────────────────────────────────────────────────────────── */
...
const GradeCard = ({ grade, index, isUrdu, isLoggedIn, onExplore }: GradeCardProps) => {
  ...
  // Collect all video IDs for this grade (subjects → chapters → videos)
  const allVideoIds = useGradeVideoIds(Number(grade.id), subjectList);

  // Real server-side progress via the v2 API
  const { watchedSet } = useVideoProgress(allVideoIds, isLoggedIn);

  // Calculate real percentage
  const progressPct = allVideoIds.length > 0
    ? Math.round((watchedSet.size / allVideoIds.length) * 100)
    : 0;
  ...
  const track = isUrdu ? (TRACKS[num]?.ur || "تعلیمی راستہ") : (TRACKS[num]?.en || "Academic Track");
    ...
    >
      ...
      <div className="mg-g-subj">
        ...
        {subjLoading ? (
          ...
          <span>
            <strong style={{ color: "#0F172A" }}>{subjectList.length}</strong>
            &nbsp;{isUrdu ? "مضامین" : subjectList.length === 1 ? "Subject" : "Subjects"}
          </span>
        )}
      </div>
        ...
        <div className="mg-subj-pills">
          {subjectList.map((s: any) => (
            <span key={s.id} className="mg-subj-pill">
              {isUrdu ? (s.urdu_name?.trim() || s.name) : s.name}
            </span>
          ))}
        </div>
      )}
      ...
      {/* ── REAL progress bar from v2 API ── */}
      <div className="mg-g-prog-wrap">
        <div className="mg-g-prog">
          <div
            className="mg-g-prog-fill"
            style={{
              width: isLoggedIn ? `${progressPct}%` : "0%",
              transition: "width 0.8s ease",
            }}
          />
        </div>
        {isLoggedIn && allVideoIds.length > 0 && (
          <span className="mg-g-prog-label">
            {progressPct}% &nbsp;·&nbsp; {watchedSet.size}/{allVideoIds.length}
            {isUrdu ? " مکمل" : " done"}
          </span>
        )}
      </div>
...
const MiddleGradesView = () => {
  ...
  const displayGrades = useMemo(() => {
    ...
    const filtered = raw.filter((g: any) => { const n = extractNum(g?.name || ""); return n >= 6 && n <= 8; });
  ...
  /* analytics — zeros until a real analytics API is wired up */
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        /* Progress bar */
        .mg-g-prog-wrap  { display:flex;flex-direction:column;gap:4px;margin-top:2px; }
        .mg-g-prog       { height:4px;background:#E2E8F0;border-radius:100px;overflow:hidden; }
        .mg-g-prog-fill  { height:100%;background:#22C55E;border-radius:100px; }
        .mg-g-prog-label { font-size:.7rem;font-weight:600;color:#64748B; }

        /* Subject pills */
        .mg-subj-pills { display:flex;flex-wrap:wrap;gap:5px;margin-top:2px; }
        .mg-subj-pill  { font-size:.65rem;font-weight:600;color:#1E40AF;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:100px;padding:3px 9px;white-space:nowrap; }
        .mg-subj-pill-skel { display:inline-block;height:20px;width:52px;background:#F1F5F9;border-radius:100px;animation:mgPulse 1.4s ease-in-out infinite; }
        ...
        .mg-feat img { position:absolute;inset:0;width:100%;height:100%;object-fit:cover; }
